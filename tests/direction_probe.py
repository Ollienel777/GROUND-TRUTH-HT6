...
directions. The former case-8 (N8) residual — a forward result with its source in
an oblique "seeded with" phrase — is now closed by the source-material origin cue
and enforced in both directions (N8/N8b) rather than tracked as a hole.
...
CASES = [
    ...
    # ---- SOURCE-MATERIAL ROLE (formerly the N8 KNOWN HOLE, now closed) ----
    # "seeded with" places the source in an oblique phrase after the destination.
    # The origin cue now recognizes it (and passive "generated by"), so a genuinely
    # forward result no longer defaults backward. Enforced in BOTH directions, per
    # the "mirror every dimension" discipline that let the first N8 regression ship.
    ("N8 forward with source in an oblique 'seeded with' phrase",
     "Fibroblast populations arose in cultures seeded with PluripotentStemCell.",
     lambda r: not _revised(r), "no revision (forward)", False),

    ("N8b backward mirror: 'seeded with' with roles swapped must still be caught",
     "PluripotentStemCell populations arose in cultures seeded with Fibroblast.",
     lambda r: _revised(r) and not r.ood_flag, "revise (down)", False),
]
...
def main():
    print("=" * 74)
    print("DIRECTION PROBE  (attacks both directions; source-material roles enforced)")
    print("=" * 74)
    hard_pass = 0
    hard_total = 0
    ...
    hard_total += len(seq)
    print("-" * 74)
    print(f"  {hard_pass}/{hard_total} enforced checks pass")
    print("=" * 74)
