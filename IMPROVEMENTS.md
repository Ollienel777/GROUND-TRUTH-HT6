...
- **Green from a suite you wrote alongside the fix is not evidence.** Adversarial
  ...
  recorded visibly rather than quietly omitted — N8 followed that discipline as an
  `XFAIL` until source-role handling closed it and promoted it to an enforced check.
...
- **Entities:** proven not to be name-coupled — the renamed-seed harness is 12/12
  identical. State mentions are graph-validated through bounded normalized n-gram
  probes, covering case, spaces, hyphens and conservative plurals without a biology
  alias table (`deterministic_perception_probe.py` locks these variants).
- **Direction:** resolved by `transition_direction` from *positive evidence only* —
  ...
  *produced* at high efficiency, emerged after…") they describe the subject rather
  ...
  Source-material roles now cover both passive production ("generated by") and the
  former N8 oblique construction ("cultures *seeded with* PSC"), with forward and
  backward mirrors enforced. The residual is no longer that named case but the open
  class of uncommon grammatical constructions that any finite deterministic role
  vocabulary can miss.
...
lexical dependency entirely. **Not applicable to this work:** the user requested a
purely deterministic, non-LLM solution. `RULES.md` conditionally
permits an organizer-provided endpoint, but this implementation neither assumes nor
uses one. This is a *production* upgrade: were it ever built, it
...
- Do not add an LLM path to this implementation — the user requested a purely
  deterministic model; keep neural perception production-only.
...
4. Skip the Bayesian rewrite for this scoring horizon. Skip the LLM tier entirely
   under the explicit deterministic-task constraint; it remains production-only.
